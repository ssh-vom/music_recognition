class Constants:
    ...
    STAFF_LINE_KERNEL_WIDTH_FRAC = (
        1 / 12
    )  # horizontal open kernel spans ~1/12 of image width to survive small gaps in printed lines
    STAFF_LINE_KERNEL_MIN_WIDTH = (
        25  # floor so the kernel is still useful on smaller images
    )
    ...
    NOTEHEAD_CLEANUP_KERNEL = (
        3,
        3,
    )  # tiny close kernel to smooth jagged edges that the open leaves behind
    NOTE_MIN_AREA_FRAC = (
        0.08  # blobs smaller than 8% of spacing^2 are dust or scanner artifacts
    )
    ...
    NOTE_MAX_SIZE_FRAC = (
        1.9  # maximum width or height; larger blobs are usually ties or slurs
    )
    NOTE_MIN_ASPECT = (
        0.45  # reject very tall/skinny blobs — those are almost always stems
    )
    ...
    NOTE_MERGE_DISTANCE_FRAC = (
        0.75  # two detected centers within 75% of spacing are averaged into one
    )
    ...
    STEP_CONFIDENCE_MEDIUM = (
        0.40  # within 40% means medium; beyond that we flag it as low confidence
    )
    ...
    FILL_ELLIPSE_X_RADIUS_FRAC = (
        0.36  # horizontal radius of the elliptical fill-test window
    )
    FILL_ELLIPSE_Y_RADIUS_FRAC = (
        0.28  # vertical radius; noteheads are wider than tall so these differ
    )
    ...
    WHOLE_NOTE_FILL_RATIO_MAX = (
        0.35  # a whole note has an open center so its ink ratio stays below 35%
    )
    ...
    STEM_MIN_RUN_FRAC = (
        1.2  # a vertical ink run must be at least 1.2x spacing tall to count as a stem
    )
    ...
    CLEF_MIN_CONFIDENCE = (
        0.15  # below this template match score we return None rather than guess
    )
    ...
    CLEF_MATCH_SCALES = (
        ...
        0.95,
        1.0,
    )  # try a few scale fractions in case the printed clef is smaller than our reference template
    ...
    BAR_SEARCH_LEFT_SKIP_OTHER_FRAC = (
        2.0  # subsequent staves only repeat the clef, so a smaller skip is enough
    )
    ...
    BAR_MIN_HEIGHT_FRAC = (
        0.4  # a valid bar line must span at least 40% of the staff height
    )
    BAR_MIN_DENSITY = 0.55  # real bar lines are nearly solid; sparse blobs are leaked noteheads or stems
    BAR_MAX_WIDTH_FRAC = (
        0.6  # anything wider than 0.6x spacing is probably not a thin bar line
    )
    BAR_DOUBLE_MIN_WIDTH_FRAC = (
        1.0  # a double bar spans at least one full spacing between its two strokes
    )
    BAR_RIGHT_MARGIN_FRAC = (
        2.0  # how close to the right staff edge counts as "near the end of the line"
    )
    BAR_LEFT_MARGIN_FRAC = (
        4.0  # similar margin on the left for detecting opening double bars
    )
    ...
    BAR_MERGE_DISTANCE_FRAC = (
        0.5  # two detected bar positions within half a spacing are merged into one
    )
    ...
    REPEAT_DOT_SEARCH_WIDTH_FRAC = (
        1.8  # how far to look on each side of the bar line for repeat dots
    )
    REPEAT_DOT_BAR_GAP_FRAC = (
        0.25  # leave a small gap between the bar line itself and the search window
    )
    REPEAT_DOT_MIN_AREA_FRAC = 0.04  # minimum dot area relative to spacing^2
    REPEAT_DOT_MAX_AREA_FRAC = (
        0.80  # maximum dot area; anything larger is probably a notehead
    )
    REPEAT_DOT_MAX_SIZE_FRAC = (
        0.95  # dots are smaller than one full spacing in each dimension
    )
    REPEAT_DOT_Y_TOLERANCE_FRAC = (
        0.65  # dots may sit up to 65% of spacing away from the expected y positions
    )
