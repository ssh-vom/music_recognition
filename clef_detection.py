...
def detect_clef(clef_key_crop: MatLike) -> ClefDetection:
    ...
    Uses two methods:
    1. Letterbox match: scales template to fill ROI height, pads horizontally
    2. Multi-scale sliding: tries multiple scales at different positions
    
    Returns clef type with confidence scores for both methods.
    """
    ...
    # Letterbox matching (full height, centered horizontally)
...
def _trim_white_border(gray: MatLike, thresh: int = CLEF_TRIM_WHITE_THRESH) -> MatLike:
    """Crop to content by removing white borders."""
...
def _prepare_roi(clef_key_crop: MatLike) -> MatLike | None:
    """Extract left portion of staff header where clef appears."""
    gray = _to_gray(clef_key_crop)
    gray = cv.bitwise_not(gray)  # Invert so clef is dark on light
...
def _empty_detection() -> ClefDetection:
    """Return detection with zero scores when ROI is invalid."""
    return ClefDetection(
        clef=None,
        confidence=0.0,
...
def _select_clef(treble_score: float, bass_score: float) -> tuple[ClefKind | None, float]:
    """Choose clef based on match scores with tiebreaker margin."""
    # Treble wins ties with small margin preference
...
def _letterbox_match(roi: MatLike, template: MatLike) -> tuple[float, tuple]:
    """Match template scaled to full ROI height, centered horizontally."""
...
def _multi_scale_match(roi: MatLike, template: MatLike) -> tuple[float, tuple]:
    """Match template at multiple scales, allowing position variation."""
    roi_h, roi_w = roi.shape[:2]
    best_score = 0.0
    best_rect = (0, 0, 0, 0)
...
def _fit_to_roi(template: MatLike, roi_h: int, roi_w: int) -> MatLike:
    """Shrink template if needed to fit within ROI."""
    th, tw = template.shape[:2]
