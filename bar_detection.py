...
def find_bars(image: MatLike, staffs: list[Staff]) -> list[BarLine]:
    """Detect all bar lines across all staves."""
    all_bars = []
...
def _find_staff_bars(image: MatLike, staff: Staff, staff_idx: int) -> list[BarLine]:
    """Detect bar lines within a single staff region."""
    y0, y1 = staff.top, staff.bottom + 1
    staff_h = y1 - y0
    roi = image[y0:y1, :]
    
    # Skip left region containing clef/key signature
    ...
    # Morphological close to join fragmented vertical lines
...
def _contours_to_bars(
    contours, staff: Staff, left_skip: int,
    staff_h: int, y0: int, y1: int, staff_idx: int
) -> list[BarLine]:
    """Convert contours to bar line objects with geometric filtering."""
    ...
    for contour in contours:
        ...
        # Edge detection for repeat signs
        ...
        # Density check (solid lines have high density)
        ...
        # Classify as single or double bar line
        if w >= min_double_width and near_right:
            # Double bar line (final barline or repeat)
            bars.append(BarLine(
                x=abs_left, y_top=y0, y_bottom=y1-1,
                kind="double_left", repeat="none", staff_index=staff_idx
            ))
            bars.append(BarLine(
                x=abs_right, y_top=y0, y_bottom=y1-1,
                kind="double_right", repeat="none", staff_index=staff_idx
            ))
        else:
            bars.append(BarLine(
                x=abs_center, y_top=y0, y_bottom=y1-1,
                kind="single", repeat="none", staff_index=staff_idx
            ))
...
def _merge_and_classify_pairs(
    ...
    """Merge close bar lines and identify double bar line pairs."""
    if len(bars) < 2:
        return bars
    ...
    # First pass: merge very close detections
    merged = [bars[0]]
    for bar in bars[1:]:
        prev = merged[-1]
    ...
    # Second pass: identify double bar line pairs at edges
    staff_right = max(line.x_end for line in staff.lines)
    left_skip = int(round(
    ...
    while i < len(merged):
        if i + 1 < len(merged):
            ...
            close_together = right.x - left.x <= pair_gap
            at_edge = right.x <= left_edge or left.x >= right_edge
            
            if left.kind == "single" and right.kind == "single" and close_together and at_edge:
